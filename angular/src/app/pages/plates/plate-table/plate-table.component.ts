...
export class PlateTableComponent {
  ...
  showModal = false; // Controla el Pop-up
  newCategoryName = ""; // Almacena el input
  errorMessage = "";
  ...
  ngOnInit() {
    this.plateService
      .selectAll()
      .subscribe((plates) => (this.plateList = plates));
    ...
    if (confirm("¿Estás seguro de que deseas eliminar este plato?")) {
      try {
        this.plateService.delete(id).subscribe({
          ...
          error: () => {
            this.errorMessage =
              "No se puede eliminar el plato por que hay pedidos asociados.";
          },
        });
      } catch (e) {
        console.error(e);
