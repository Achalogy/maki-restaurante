...
/**
 * DeliveryController Expone los endpoints REST para la gestión de domiciliarios. Base path:
 * /api/v1/delivery
 */
@RestController
...
public class DeliveryController {
    ...
    /**
     * Actualiza un domiciliario existente. Solo actualiza los campos que vienen en el body (patch
     * parcial). Útil para el toggle de disponibilidad (solo envía available).
     */
    @PostMapping("/{id}")
    public ResponseEntity<Delivery> updateDelivery(
            @PathVariable Long id, @RequestBody(required = false) Delivery data) {
        try {
            ...
            if (data.getNational_id() != null) existing.setNational_id(data.getNational_id());
