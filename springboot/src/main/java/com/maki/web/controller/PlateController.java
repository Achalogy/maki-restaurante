...
public class PlateController {
    ...
    @Autowired private PlateService platoService;
    ...
    public ResponseEntity<Plate> getPlateById(@PathVariable Long id) {
        try {
            return new ResponseEntity<>(platoService.selectById(id), HttpStatus.OK);
        } catch (Exception e) {
    ...
    // CREATE
    @PostMapping("")
    public ResponseEntity<PlateDTO> savePlate(
            @RequestBody Plate plato, @RequestParam Long categoryId) {
        try {
            Category category = categoryService.selectById(categoryId);
            if (category == null) return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
            plato.setCategory(category);
            return new ResponseEntity<>(
                    mapper.toPlateDTO(platoService.insert(plato)), HttpStatus.OK);
        } catch (Exception e) {
    ...
    public ResponseEntity<PlateDTO> updatePlate(
        ...
        try {
            ...
            return new ResponseEntity<>(
                    mapper.toPlateDTO(platoService.update(updateData)), HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
        }
    }
    ...
    public ResponseEntity<Boolean> updatePlateCategory(
            @PathVariable Long platoId, @PathVariable Long categoryId) {
        try {
            Category category = categoryService.selectById(categoryId);
