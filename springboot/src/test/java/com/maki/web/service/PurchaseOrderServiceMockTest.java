...
/**
 * Pruebas unitarias con Mocks para PurchaseOrderServiceImpl. Se utiliza Mockito para simular el
 * comportamiento de los repositorios y servicios dependientes.
 */
@ExtendWith(MockitoExtension.class)
public class PurchaseOrderServiceMockTest {

    @Mock private PurchaseOrderRepository repo;

    @Mock private OrderDetailsRepository orderDetailsRepo;

    @Mock private ClientService clientService;

    @Mock private AdditionalOrderDetailsRepository additionalOrderDetailsRepo;
    ...
    void setUp() {
        testClient =
                new Client("Juan", "Perez", "juan@example.com", "123", "555", "Calle Falsa 123");
        testClient.setId(1L);
    ...
    void insert_ValidOrder_ReturnsSavedOrder() throws EntityConstraintException {
        ...
        when(repo.save(any(PurchaseOrder.class)))
                .thenAnswer(
                        invocation -> {
                            PurchaseOrder saved = invocation.getArgument(0);
                            saved.setId(10L); // simulamos que el service asigna una id
                            return saved;
                        });
        ...
        assertEquals(10L, result.getId()); // revisar que se haya asignado la id
