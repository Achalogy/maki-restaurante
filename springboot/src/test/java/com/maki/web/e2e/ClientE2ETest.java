...
@Disabled("E2E pendiente: los selectores no coinciden con el HTML actual")
@ActiveProfiles("test")
...
public class ClientE2ETest {
    ...
    /**
     * El usuario llega al login, ingresa sus credenciales correctas y debe ser redirigido a su
     * página de sesión (/client/{id}).
     */
    @Test
    @Order(1)
    public void caso1_login_ConCredencialesValidas_RedireccionaAlPerfil() {
        ...
        WebElement textoEmail =
                driver.findElement(By.xpath("//*[contains(text(), '" + CLIENT_EMAIL + "')]"));
    ...
    /**
     * Prueba negativa: credenciales incorrectas deben mostrar una alerta. (El servicio devuelve 400
     * y el componente llama a alert())
     */
    @Test
    @Order(2)
    public void caso1_login_ConPasswordIncorrecta_MuestraAlerta() {
        ...
        driver.findElement(By.id("username")).sendKeys(CLIENT_EMAIL);
        driver.findElement(By.id("password")).sendKeys("password_incorrecta_123");
        driver.findElement(By.id("logInAdButton")).click();
        ...
        try {
            ...
            assertTrue(
                    errorMessage.isDisplayed(), "Debe mostrar mensaje de credenciales incorrectas");
    ...
    /**
     * El usuario (ya logueado) va al menú, hace clic en los primeros 2 platos disponibles,
     * selecciona 2 adicionales en cada uno, los agrega al carrito y verifica: - Que el carrito
     * tiene exactamente 2 ítems. - Que cada ítem tiene 2 adicionales visibles. - Que el total del
     * carrito es mayor a 0 (no hardcodeado).
     */
    @Test
    @Order(3)
    public void caso2_menu_Agregar2Platoscon2AdicionalesYVerificarCarrito() {
        ...
        wait.until(
                ExpectedConditions.presenceOfAllElementsLocatedBy(By.className("btn-ver-plato")));

        List<WebElement> botonesVerPlato = driver.findElements(By.className("btn-ver-plato"));

        assertTrue(botonesVerPlato.size() >= 2, "Debe haber al menos 2 platos en el menú");
        ...
        wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.className("carrito-item")));

        List<WebElement> itemsEnCarrito = driver.findElements(By.className("carrito-item"));

        // ASSERT 1: El carrito debe tener exactamente 2 ítems
        assertEquals(2, itemsEnCarrito.size(), "El carrito debe contener exactamente 2 platos");

        // ASSERT 2: Cada ítem debe mostrar exactamente 2 adicionales
        for (WebElement item : itemsEnCarrito) {
            List<WebElement> adicionalesDelItem =
                    item.findElements(By.className("carrito-item-adicional"));
            assertEquals(
                    2,
        ...
        WebElement elementoTotal =
                wait.until(ExpectedConditions.presenceOfElementLocated(By.id("carrito-total")));
    ...
    /**
     * Flujo completo del sprint: 1. Login del cliente. 2. Ir al menú y agregar 2 platos con 2
     * adicionales cada uno. 3. Verificar el carrito (ítems, nombres, adicionales). 4. Confirmar el
     * pedido ("Ir a pagar"). 5. Ir al historial de pedidos del cliente. 6. Verificar que el pedido
     * aparece con los platos y adicionales correctos. 7. Verificar que el total del historial es
     * mayor a 0 (no hardcodeado).
     */
    @Test
    @Order(4)
    public void caso3_flujoCompleto_LoginAgregarPedidoYVerificarHistorial() {
        ...
        List<WebElement> nombresEnCarrito =
                wait.until(
                        ExpectedConditions.presenceOfAllElementsLocatedBy(
                                By.className("carrito-item-nombre")));

        List<String> nombresTexto =
                nombresEnCarrito.stream().map(y -> y.getAttribute("textContent")).toList();
        ...
        WebElement elementoTotalCarrito =
                wait.until(ExpectedConditions.presenceOfElementLocated(By.id("carrito-total")));
        ...
        long totalCarritoNumerico = Long.parseLong(textoTotalCarrito.replaceAll("[^0-9]", ""));

        assertTrue(totalCarritoNumerico > 0, "El total del carrito debe ser mayor a 0");

        // ── PASO 7: Confirmar el pedido ───────────────────────────────────────
        WebElement btnPagar =
                wait.until(ExpectedConditions.elementToBeClickable(By.id("btn-pagar")));
        btnPagar.click();
        ...
        wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.className("orden-card")));
        ...
        List<WebElement> itemsDeOrden = ultimaOrden.findElements(By.className("orden-item-nombre"));

        List<String> nombresItemsOrden = itemsDeOrden.stream().map(WebElement::getText).toList();
        ...
        List<WebElement> todosLosItems = ultimaOrden.findElements(By.className("orden-item"));

        for (WebElement item : todosLosItems) {
            List<WebElement> adicionalesDelItem =
                    item.findElements(By.className("orden-adicional-tag"));
            assertTrue(
        ...
        List<WebElement> totalesDeOrdenes = ultimaOrden.findElements(By.className("orden-total"));
        ...
        long totalHistorialNumerico = Long.parseLong(textoTotalHistorial.replaceAll("[^0-9]", ""));

        assertTrue(
                totalHistorialNumerico > 0,
                "El total del historial debe ser mayor a 0. Encontrado: " + textoTotalHistorial);
    }
    ...
    /**
     * Hace clic en el botón del plato indicado, selecciona 2 adicionales, lo agrega al pedido y
     * retorna el nombre del plato leído desde la página del plato (id="plate-name"). Esto garantiza
     * que el nombre capturado corresponde exactamente al plato abierto, evitando desajustes de
     * índice entre plate-card-name y btn-ver-plato en el menú.
     *
     * @param botonesVerPlato Lista de botones "+" del menú
     * @param indicePlato Posición (0-based) del plato a abrir
    ...
    private String agregarPlatoConAdicionalesYObtenerNombre(
            List<WebElement> botonesVerPlato, int indicePlato) {
        ...
        List<WebElement> checkboxes = driver.findElements(By.className("additional-checkbox"));
        ...
        WebElement btnAgregar =
                wait.until(ExpectedConditions.elementToBeClickable(By.id("btn-add-to-order")));
        btnAgregar.click();
