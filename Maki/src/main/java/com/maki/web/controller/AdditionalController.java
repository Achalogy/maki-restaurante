...
public class AdditionalController {
    ...
    public List<AdditionalDTO> getAllAdicionales(@RequestParam(required = false) Long categoryId) {
        ...
        if (categoryId != null) {
            additionals = additionalCategoryService.findByCategory_Id(categoryId)
                    .stream().map(AdditionalCategory::getAdditional).collect(Collectors.toList());
        } else {
    ...
    // Las categorías siguen retornando la estructura completa (se necesita para la vista de edición)
    @GetMapping("/categories")
    public List<AdditionalCategory> getAllAdditionalCategories(
            @RequestParam(required = false) Long categoryId,
            @RequestParam(required = false) Long additionalId) {
        if (categoryId == null && additionalId == null) return additionalCategoryService.selectAll();
    ...
    @PostMapping("")
    public ResponseEntity<AdditionalDTO> createAdditional(@RequestBody(required = false) Additional data) {
        try {
            return new ResponseEntity<>(mapper.toAdditionalDTO(additionalService.insert(data)), HttpStatus.OK);
        } catch (Exception e) {
            if (e instanceof EntityNotFoundException) return new ResponseEntity<>(HttpStatus.NOT_FOUND);
    ...
    public ResponseEntity<AdditionalDTO> getAdditionalById(@PathVariable Long id) {
        try {
            return new ResponseEntity<>(mapper.toAdditionalDTO(additionalService.selectById(id)), HttpStatus.OK);
        } catch (Exception e) {
    ...
    @PostMapping("/{id}")
    public ResponseEntity<AdditionalDTO> updateAdicional(@PathVariable Long id,
        ...
        try {
            ...
            return new ResponseEntity<>(mapper.toAdditionalDTO(additionalService.update(updateData)), HttpStatus.OK);
        } catch (Exception e) {
            if (e instanceof EntityNotFoundException) return new ResponseEntity<>(HttpStatus.NOT_FOUND);
            return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
        }
