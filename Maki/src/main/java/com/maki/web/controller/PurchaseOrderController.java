...
public class PurchaseOrderController {
  ...
  public List<PurchaseOrder> getAllPurchaseOrders(
    @RequestParam(required = false) Optional<Long> notCompleted
  ) {
  ...
  @GetMapping("/{id}")
  public ResponseEntity<PurchaseOrder> getPurcharseOrderById(
    @PathVariable Long id
  ) {
    try {
      PurchaseOrder purchaseOrder = purchaseOrderService.selectById(id);
      if (purchaseOrder == null) return new ResponseEntity<>(
  ...
  public ResponseEntity<PurchaseOrderDTO> createPurchaseOrder(
    ...
    try {
      PurchaseOrder order = purchaseOrderService.createPurchaseOrderFromcart(
        client.getId(),
        plates
      );
      return new ResponseEntity<>(
        mapper.toPurchaseOrderDTO(order),
        HttpStatus.OK
      );
    } catch (Exception err) {
      err.printStackTrace();
  ...
  public ResponseEntity<PurchaseOrderDTO> updatePurchaseOrder(
    ...
    try {
      ...
      if (newDelivery != null) {
        ...
        if (
          data.getStatus().equals("cancelled") ||
      ...
      if (data.getStatus() != null) {
        if (
          !updateData.getStatus().equals("sent") &&
          data.getStatus().equals("sent")
        ) {
      ...
      if (data.getDelivery_date() != null) updateData.setDelivery_date(
        data.getDelivery_date()
      );

      return new ResponseEntity<>(
        mapper.toPurchaseOrderDTO(purchaseOrderService.update(updateData)),
        HttpStatus.OK
      );
    } catch (Exception e) {
      return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
