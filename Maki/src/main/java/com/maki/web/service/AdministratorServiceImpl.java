...
/**
 * CORRECCIÓN: Se implementa verifyCredentials que faltaba.
 * NOTA: Los admins tienen contraseña en UserEntity (encriptada),
 * pero también se verifica contra la contraseña plana del objeto Administrator
 * que viene del frontend. Spring Security ya maneja el login real via JWT;
 * este método se usa solo para el login "legacy" por el endpoint /admin/log-in.
 *
 */
@Service
    ...
    public Administrator verifyCredentials(String username, String password)
        ...
        Administrator admin = repo.findByUsername(username)
                .orElseThrow(() -> new EntityNotFoundException(
                        "No existe un administrador registrado con el username: " + username));
