  ...
  SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
    ...
    http.csrf(AbstractHttpConfigurer::disable)
        ...
        .authorizeHttpRequests(requests -> requests
            ...
            .requestMatchers(HttpMethod.GET, "/api/v1/plate/**").permitAll()
            .requestMatchers(HttpMethod.GET, "/api/v1/category/**").permitAll()
            .requestMatchers(HttpMethod.GET, "/api/v1/additional/**").permitAll()
            ...
            .requestMatchers(HttpMethod.POST, "/api/v1/purchase-order/client/**").hasAuthority("CLIENT")
            .requestMatchers("/api/v1/client/**").hasAnyAuthority("CLIENT", "ADMIN")

            // Acceso OPERADOR y ADMIN (Gestión de pedidos y entregas)
            .requestMatchers("/api/v1/delivery/**").hasAnyAuthority("OPERATOR", "ADMIN")
            .requestMatchers("/api/v1/order-details/**").hasAnyAuthority("OPERATOR", "ADMIN")
            .requestMatchers(HttpMethod.POST, "/api/v1/purchase-order/{id}").hasAnyAuthority("OPERATOR", "ADMIN")
            .requestMatchers(HttpMethod.GET, "/api/v1/purchase-order/**").hasAnyAuthority("CLIENT", "OPERATOR", "ADMIN")
