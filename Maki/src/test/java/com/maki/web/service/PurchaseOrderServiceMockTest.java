...
public class PurchaseOrderServiceMockTest {
  ...
  void createPurchaseOrderFromcart_Success() {
    ...
    when(orderDetailsRepo.save(any(OrderDetails.class))).thenAnswer(i ->
      i.getArgument(0)
    );

    // Act
    PurchaseOrder result = purchaseOrderService.createPurchaseOrderFromcart(
      1L,
      cart
    );
  ...
  void selectNotCompleted_FiltersCorrectly() {
    ...
    when(repo.findAll()).thenReturn(
      Arrays.asList(completedOrder, pendingOrder)
    );
